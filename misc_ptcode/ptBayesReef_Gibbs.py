...
class ptReplica(multiprocessing.Process):
	...
	def run(self):
		...
		#f=open(self.filename + '/posterior/'+ str(self.temperature)+'_res.txt','a')

		#fx=open(self.filename + '/posterior/'+ str(self.temperature)+'_res_.txt','a+')

		#with open(self.filename + '/posterior/'+ str(self.temperature)+'_res.txt', "a") as myfile:
		...
		for samp in range(samples-1):
			...
			for gx in range(0, 7):
				...
				#temp_proposal = self.realvalues_vec.copy()
				...
				v_proposal = temp_proposal.copy()  #np.concatenate((p_sed1,p_sed2,p_sed3,p_sed4,p_flow1,p_flow2,p_flow3,p_flow4))
				...
				save_res =  np.array([samp,gx, num_accepted, likelihood_proposal, diff])


				f=open(self.filename + '/posterior/'+ str(self.temperature)+'_res_.txt','a')  
				sx = np.array2string(save_res, formatter={'float_kind':lambda save_res: "%.2f" % save_res})
				...
				if u < mh_prob: # Accept sample
					...
					#b = i
					pos_likl[samp + 1, 1] = pos_likl[samp,1]  
					pos_param[samp+1,:] = pos_param[samp,:] 
			...
			fx=open(self.filename + '/posterior/'+ str(self.temperature)+'_param.txt','a') 
			params_save =  pos_param[samp+1,:] 
			params_temp = np.array2string(params_save, formatter={'float_kind':lambda params_save: "%.6f" % params_save}) 
			fx.write(params_temp+"\n")

			#np.savetxt(f,pos_param[samp+1,:], fmt='%1.7f') 
...
def main():
	...
	np.savetxt(likelihood, fname+'lhood.txt')
	print(likelihood)
	...
	plt.clf()
	np.savetxt(accept_list, fname+'accept.txt')
	print(accept_list)
